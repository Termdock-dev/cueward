# Background keyboard input and scrolling
...
The opt-in desktop tests use disposable AppKit receivers to verify text, keys, shortcuts, non-key-window scrolling, wrong-window rejection, and interrupted text delivery with paired key releases. They do not establish compatibility with every app or isolation during physical typing in a foreground app.
