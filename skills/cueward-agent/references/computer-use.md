...
Keyboard binding can use a native window ID when a dialog's AX title is absent or differs from its capture title. The owning process and bounds must still match; unavailable native lookup uses the existing unique title-and-bounds check. Take a separate snapshot of the actual focused dialog. A parent window's target is not a substitute, and successful binding does not prove the dialog accepted the input.
...
Use `window wait --target '<snapshot input_target>' --condition <condition>` after a state-changing action. Element conditions require `--role`; narrow with exact `--name` or `--identifier` from inspection. Conditions are `element-exists`, `element-absent`, `enabled`, `value-equals --value <expected>`, and `window-gone` without selectors. The result distinguishes a match, timeout, changed/gone window, and ambiguous selector. Partial trees cannot establish absence or uniqueness. After waiting, inspect again for fresh action targets; never replay uncertain actions automatically. A matching UI condition alone does not prove the overall task or external artifact succeeded.
...
`receiver_pid` identifies the actual AX process, which may differ from the host. Both must remain in the background for App actions. Targets bind process instances, current window context, and observed ancestors; they expire after five minutes. Secure and disabled nodes have no target. Partial reads also suppress targets unless only the optional `AXDescription` is unavailable. `unavailable_attributes` means the corresponding attribute is unknown, not empty; inspect relevant descendants when a target is absent. Unreadable security metadata stops inspection before field values are read. App inspection requires an unlocked session and Accessibility permission.
